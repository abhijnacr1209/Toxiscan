"""
Fixed Metabolism Site Animation for Toxiscan
Simplified working animations showing metabolic transformations
"""
...
class MetabolismAnimator:
    ...
    def create_simple_metabolism_animation(self, smiles: str, predicted_sites: List[Dict]) -> go.Figure:
        """Create a simple working animation of metabolic transformations"""
        ...
        try:
            ...
            # Create frames for each metabolic site
    ...
    def create_metabolic_pathway_chart(self, predicted_sites: List[Dict]) -> go.Figure:
        ...
        for site in predicted_sites:
            for pathway in site.get('metabolic_pathways', ['aromatic_oxidation']):
                pathway_info = self.metabolic_pathways.get(pathway, self.metabolic_pathways['aromatic_oxidation'])
                pathways[pathway_info['name']] = pathway_info['probability']
        ...
        fig.add_trace(go.Bar(
            ...
            marker_color=['#FF6B6B', '#4ECDC4', '#45B7D1', '#96CEB4', '#FFEAA7', '#DDA0DD'][:len(pathways)],
...
def create_metabolism_animation_tab(smiles: str, predicted_sites: List[Dict]):
    ...
    # Check if we have sites to animate
    ...
    with st.spinner("Creating metabolism animation..."):
        animation_fig = animator.create_simple_metabolism_animation(smiles, predicted_sites)
...
def create_simple_animation_demo():
    """Create a simple demonstration animation that always works"""
    
    st.markdown("### 🎬 Simple Metabolism Animation Demo")
    st.markdown("""
    <div class="feature-card">
        <strong>🎬 Demo Animation:</strong> A working example of metabolism site animation
        <br><strong>🔬 Sample Molecule:</strong> Aspirin metabolism demonstration
        <br><strong>⏱️ 3 Steps:</strong> Shows aromatic oxidation and conjugation pathways
    </div>
    """, unsafe_allow_html=True)
    ...
    demo_sites = [
        {
            ...
            'risk_score': 0.8,
            'confidence': 0.75,
            'metabolic_pathways': ['aromatic_oxidation']
        },
        {
            'atom_index': 5,
            'atom_symbol': 'C',
            'risk_score': 0.6,
            'confidence': 0.65,
            'metabolic_pathways': ['conjugation']
        },
        {
            'atom_index': 8,
            'atom_symbol': 'O',
    ...
    animator = MetabolismAnimator()
    animation_fig = animator.create_simple_metabolism_animation("CC(=O)Oc1ccccc1C(=O)O", demo_sites)
    
    st.plotly_chart(animation_fig, key="demo_animation")
    
    # Show pathway info
    st.markdown("#### 🔬 Demo Pathway Information")
    
    for site in demo_sites:
        with st.expander(f"🔥 Site {site['atom_index']} ({site['atom_symbol']}) - Risk {site['risk_score']:.2f}"):
            for pathway in site['metabolic_pathways']:
                pathway_info = animator.metabolic_pathways.get(pathway, animator.metabolic_pathways['aromatic_oxidation'])
