...
with PDF Download, GNN Explanations, 3D Models, and Heat Maps
"""
...
class Molecular3DVisualizer:
    """Enhanced 3D molecular visualization with heatmap"""
    
    @staticmethod
    def create_3d_molecule_with_heatmap(smiles: str, highlights: Dict = None, atom_importance: Dict = None) -> go.Figure:
            ...
            for atom in mol.GetAtoms():
                ...
                if atom_importance and atom_idx in atom_importance:
                    ...
                    if importance > 0.7:
                        ...
                        color = 'lightgreen'
                        size = 8
                elif highlights and atom_idx in highlights:
                    color = 'red'
                    size = 12
                else:
                    color = 'lightblue'
    ...
    @staticmethod
    def create_toxicity_heatmap(smiles: str, atom_importance: Dict = None) -> go.Figure:
        ...
        try:
            ...
            fig.add_trace(go.Heatmap(
                z=[[importance_values[i] for i in range(len(atom_symbols))],
                x=atom_symbols,
                y=['Importance'],
                colorscale='Reds',
                showscale=True,
...
class GNNExplainer:
    """Enhanced GNN explainer with detailed explanations and 3D visualization"""
    ...
    def _load_toxicity_patterns(self):
        ...
        return {
            ...
            'aryl_amine': {
                'toxicity_score': 0.7,
                'description': 'Aromatic amines can be metabolically activated to form reactive nitroso compounds',
    ...
    def explain_toxicity(self, smiles: str) -> Dict[str, Any]:
        ...
        try:
            ...
            for pattern_name, pattern_info in self.toxicity_patterns.items():
                # Simplified pattern detection based on SMILES
                if pattern_name == 'nitro_aromatic' and ('N(' in smiles or 'N+(' in smiles):
                ...
                elif pattern_name == 'epoxide_formation' and '[CH2]=[CH]' in smiles:
                    detected_patterns.append({
                        'pattern_name': pattern_name,
                        'toxicity_score': pattern_info['toxicity_score'],
                        'description': pattern_info['description'],
                ...
                elif pattern_name == 'quinone_formation' and 'c1ccc(cc1)C=O' in smiles:
                    detected_patterns.append({
                        'pattern_name': pattern_name,
                        'toxicity_score': pattern_info['toxicity_score'],
                        'description': pattern_info['description'],
...
def get_toxiscan_components():
    ...
    return visualizer, gnn_explainer

# Main application
def main():
    visualizer, gnn_explainer = get_toxiscan_components()
    ...
    if smiles:
        ...
        with tab4:
            ...
            if 'error' in gnn_results:
                ...
                mol_3d_fig = visualizer.create_3d_molecule_with_heatmap(
                    smiles, 
                    atom_importance=atom_importance
                )
                ...
                heatmap_fig = visualizer.create_toxicity_heatmap(
                    smiles,
                    atom_importance=atom_importance
                )
...
Most Toxic Subgraph: {gnn_results.get('most_toxic_subgraph', {}).get('pattern_name', 'None') if gnn_results.get('most_toxic_subgraph') else 'None')}
